...
const pagePath = {
    asc: '#/introduction/asc?eventid=catalyst_test&testmap=webglmap',
}
    ...
    t.describe('ASCScale', function () {
        ...
        t.it('Turbulence_on', function (done) {
            driver.get('https://www.yahoo.co.jp')
                ...
                .then(() => driver.findElement(By.id('webglzoomoutbt')).click()) // @test mapズームアウト
                .then(() => driver.findElement(By.id('webglzoomoutbt')).click()) // @test mapズームアウト
        ...
        t.it('validtime_change', function (done) {
            driver.findElement(By.xpath('//div[@id="root"]/div/div[2]/div/div/div[1]/div/div/div/div/div/div/div[3]')).click() // @test TurbulenceをON
                ...
                .then(() => common.checkstatus())
                .then(() => driver.sleep(1000))
                .then(() => driver.findElement(By.id('webglzoominbt')).click()) // @test mapズームイン
                .then(() => common.checkstatus())
        ...
        t.it('level_change', function (done) {
            driver.findElement(By.xpath(`//div[@id="root"]/div/div[2]/div/div/div[1]/div/div/div/div/div/div/div[2]/select[1]/option[${level_index}]`)).click() // @test levelの下限を100に設定
                .then(() => common.checkstatus())
                .then(() => driver.sleep(1000))
                .then(() => driver.actions().mouseMove(driver.findElement(By.id('map')), { x: 800, y: 100 }).mouseDown().mouseMove({ x: -500, y: 0 }).perform()) // @test mapを右にスライド
                .then(() => driver.actions().mouseUp().perform())
                .then(() => common.checkstatus())
