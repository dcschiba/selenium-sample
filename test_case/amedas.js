...
const pagePath = {
    amedas: '#/introduction/amedas?eventid=catalyst_test&testmap=webglmap',
}
exports.test = function (driver, t) {
    ...
    t.describe('AMeDAS', function () {
        ...
        t.it('AMeDAS_zoom_out', function (done) {
            ...
            driver.findElement(By.id('webglzoomoutbt')).click() // @test mapズームアウト
        ...
        t.it('AMeDAS_move_area', function (done) {
            driver.findElement(By.id('webglzoomoutbt')).click() // @test mapズームアウト
            driver.sleep(1000)
                .then(() => driver.actions().mouseMove(driver.findElement(By.id('map')), { x: 500, y: 100 }).mouseDown().mouseMove({ x: -200, y: 200 }).perform()) // @test mapを移動
